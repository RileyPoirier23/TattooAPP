...
const StatCard: React.FC<{ title: string; value: number | string, icon: React.ReactNode }> = ({ title, value, icon }) => (
    <div className="bg-white dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-800 flex items-center space-x-4">
...
const TableCard: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <div className="bg-white dark:bg-gray-900/50 rounded-2xl border border-gray-200 dark:border-gray-800">
        <h3 className="text-xl font-bold text-brand-dark dark:text-white p-4 border-b border-gray-200 dark:border-gray-800">{title}</h3>
                ...
                <TableCard title="Pending Verification Requests">
                    <Table headers={['Request ID', 'Name', 'Type', 'Submitted', 'Actions']}>
                        {pendingVerifications.map(req => (
                            <tr key={req.id} className="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800/50">
                                <td className="px-6 py-4 font-mono text-xs">{req.id}</td>
                                <td className="px-6 py-4 font-medium text-brand-dark dark:text-white">{req.itemName}</td>
