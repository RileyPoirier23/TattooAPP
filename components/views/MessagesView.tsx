...
export const MessagesView: React.FC<MessagesViewProps> = ({ conversationId, navigate }) => {
    ...
    return (
        <div className="max-w-7xl mx-auto h-[calc(100vh-120px)] flex bg-white dark:bg-gray-900/50 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
            {/* Left Panel: Conversations List */}
            <div className={`w-full md:w-1/3 border-r border-gray-200 dark:border-gray-800 flex-col ${isChatVisibleMobile ? 'hidden md:flex' : 'flex'}`}>
                <header className="p-4 border-b border-gray-200 dark:border-gray-800">
                    <h2 className="text-xl font-bold text-brand-dark dark:text-white">Conversations</h2>
                </header>
                <div className="overflow-y-auto flex-grow">
                    {conversations.length > 0 ? conversations.map(convo => (
                        <button
                            ...
                            className={`w-full text-left p-4 flex items-center space-x-3 transition-colors ${activeConversationId === convo.id ? 'bg-brand-secondary/20' : 'hover:bg-gray-100 dark:hover:bg-gray-800/50'}`}
                        >
                            <UserCircleIcon className="w-10 h-10 text-brand-gray flex-shrink-0" />
                            <div>
                                <p className="font-semibold text-brand-dark dark:text-white">{convo.otherUser.name}</p>
                            </div>
                        </button>
                    )) : (
                         <div className="p-8 text-center text-brand-gray">
                            <h3 className="font-semibold text-brand-dark dark:text-white">No Conversations Yet</h3>
            ...
            <div className={`w-full md:w-2/3 flex-col ${isChatVisibleMobile ? 'flex' : 'hidden md:flex'}`}>
                ...
                {!activeConversationId && !isLoading && (
                     <div className="flex-grow flex flex-col items-center justify-center text-center p-4">
                        <PaperAirplaneIcon className="w-16 h-16 text-gray-400 dark:text-gray-700 mb-4" />
                        <h3 className="text-xl font-semibold text-brand-dark dark:text-white">Select a Conversation</h3>
                        ...
                        <div className="flex-grow p-4 overflow-y-auto space-y-4">
                            {messages.map(msg => (
                                <div key={msg.id} className={`flex ${msg.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-md p-3 rounded-2xl ${msg.senderId === user.id ? 'bg-brand-primary text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 text-brand-dark dark:text-brand-light rounded-bl-none'}`}>
                                        {msg.attachmentUrl && (
                         ...
                         <footer className="p-4 border-t border-gray-200 dark:border-gray-800">
                            ...
                            <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
                                ...
                                <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2 text-brand-gray hover:text-brand-dark dark:hover:text-white transition-colors">
                                ...
                                <input
                                    ...
                                    className="flex-grow bg-gray-100 dark:bg-gray-800 rounded-full py-2 px-4 text-brand-dark dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-primary"
                                />
