...
import type { Shop, Booth, ModalState, Booking, Artist } from '../../types';
...
export const ShopOwnerDashboard: React.FC<ShopOwnerDashboardProps> = ({
  ...
  bookings,
  artists,
  // updateShop, // Removed as we aren't using it for payments anymore in this view
  addBooth,
  deleteBooth,
  openModal,
}) => {
        ...
        <div className="bg-white dark:bg-gray-900/50 rounded-2xl p-6 border border-gray-200 dark:border-gray-800">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
              {shop.amenities.map(a => (
                  <span key={a} className="text-xs bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded text-brand-dark dark:text-white">{a}</span>
              ))}
          </div>
        </div>

        <div className="bg-brand-primary/10 rounded-2xl p-6 border border-brand-primary/20">
          <h3 className="text-lg font-bold text-brand-dark dark:text-white mb-2">Shop Wallet</h3>
          <p className="text-brand-gray text-sm mb-4">All bookings are processed securely through InkSpace. Your earnings are automatically tracked here.</p>
          {/* Placeholder for future Stripe Connect integration */}
          <button className="w-full bg-brand-primary text-white font-bold py-2 rounded-lg opacity-50 cursor-not-allowed" disabled>
        ...
        <div className="bg-white dark:bg-gray-900/50 rounded-2xl p-6 border border-gray-200 dark:border-gray-800">
          ...
          <div className="space-y-4">
              {booths.length > 0 ? booths.map(booth => (
                  ...
                  <p className="text-brand-gray italic">No booths listed yet.</p>
              )}
          </div>
           <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-800">
               ...
               <div className="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <button onClick={handleAddBooth} className="w-full sm:w-auto bg-brand-secondary text-white font-bold py-3 px-6 rounded-lg">Add</button>
               </div>
          </div>
        </div>
